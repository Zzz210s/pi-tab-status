...
- `‖ Windows PowerShell (confirm)`: waiting for user confirmation/input (e.g. permission-gate dialogs)
...
| waiting for user | `ui_prompt_start` / `ui_prompt_end` | `‖` + prompt kind | paused (4) |
...
- `extensions/tab-status/render.ts` - pure rendering: view -> title string, spinner frames, shell-label detection, OSC progress sequences. All glyphs are non-emoji monospace characters.
